...
<template>
  <main class="mx-auto max-w-7xl px-4 py-8 sm:px-6">
    ...
    <form class="mt-6 grid gap-3 rounded-2xl bg-white p-4 ring-1 ring-slate-200 md:grid-cols-4" @submit.prevent="load">
      <select v-model="filters.variant_id" class="rounded-xl border border-slate-300 bg-white px-3 py-2.5">
        <option value="">Mọi biến thể</option>
        <option v-for="balance in store.inventory" :key="balance.variant_id" :value="balance.variant_id">
          {{ balance.product_name }} · {{ balance.sku }}
        </option>
      </select>
      <input v-model="filters.date_from" class="rounded-xl border border-slate-300 px-3 py-2.5" type="datetime-local" />
      <input v-model="filters.date_to" class="rounded-xl border border-slate-300 px-3 py-2.5" type="datetime-local" />
        ...
        <table class="w-full min-w-[1000px] text-left text-sm">
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="movement in store.movements" :key="movement.id">
              ...
              <td class="px-4 py-4"><strong>{{ movement.sku }}</strong></td>
            ...
            <tr v-if="!store.loading && !store.movements.length">
              <td class="px-5 py-12 text-center text-slate-500" colspan="7">Chưa có biến động kho.</td>
            </tr>
          </tbody>
        </table>
      </div>
