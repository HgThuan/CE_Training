  ...
  <main class="mx-auto max-w-7xl px-4 py-8 sm:px-6">
    <div class="flex flex-wrap items-end justify-between gap-5">
      ...
      <div class="flex flex-wrap gap-2">
        <RouterLink class="rounded-xl border bg-white px-4 py-2.5 font-bold" to="/seller/inventory/entries">
          Phiếu nhập
        </RouterLink>
        <RouterLink class="rounded-xl border bg-white px-4 py-2.5 font-bold" to="/seller/inventory/out">
          Xuất / kiểm kê
        </RouterLink>
        <RouterLink class="rounded-xl bg-slate-950 px-4 py-2.5 font-bold text-white" to="/seller/inventory/movements">
          Sổ kho
        </RouterLink>
      </div>
    </div>
    ...
    <div class="mt-6 flex items-center justify-between rounded-2xl bg-white p-4 ring-1 ring-slate-200">
          ...
          <tbody class="divide-y divide-slate-100">
            ...
            >
              <td class="px-5 py-4">
                ...
                <span class="text-xs text-slate-500">{{ balance.sku }} · {{ balance.variant_name }}</span>
              </td>
              <td
    ...
    <div v-if="selected" class="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4">
      <form class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl" @submit.prevent="saveThreshold">
        ...
        <div class="mt-5 flex justify-end gap-3">
          <button class="rounded-xl border px-4 py-2.5 font-bold" type="button" @click="selected = null">
            Hủy
          </button>
          <button class="rounded-xl bg-indigo-600 px-4 py-2.5 font-bold text-white" :disabled="saving">
            Lưu
          </button>
        </div>
      </form>
