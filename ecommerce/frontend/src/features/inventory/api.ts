...
export const inventoryApi = {
  ...
  updateStockEntry: (entryId: number, payload: Partial<StockEntryPayload>) =>
    http.patch<ApiResponse<StockEntry>>(
      `/seller/inventory/stock-entries/${entryId}/`,
      payload,
    ),
  confirmStockEntry: (entryId: number) =>
    http.post<ApiResponse<StockEntry>>(
      `/seller/inventory/stock-entries/${entryId}/confirm/`,
    ),
  ...
  updateThreshold: (variantId: string, lowStockThreshold: number) =>
    http.post<ApiResponse<InventoryBalance>>(
      `/seller/inventory/${variantId}/threshold/`,
