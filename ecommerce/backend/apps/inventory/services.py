...
class StockService:
    ...
    def _apply_delta(
        *,
        ...
        field = "available_stock" if bucket == StockMovement.Bucket.AVAILABLE else "reserved_stock"
    ...
    def confirm_stock_entry(stock_entry_id, user) -> StockEntry:
        shop = _get_seller_shop(user)
        entry = StockEntry.objects.select_for_update().filter(pk=stock_entry_id, shop=shop).first()
        if entry is None:
            raise BusinessError("Không tìm thấy phiếu nhập kho", http_status=404)
        ...
        items = list(
            entry.items.select_related("variant__product", "variant__shop").order_by("variant_id")
        )
        if not items:
    ...
    def _out_items(*, shop: Shop, entry_type: str, raw_items: list[dict]) -> list[dict]:
        ...
        for item in raw_items:
            ...
            if quantity < 0 or (entry_type == StockOutEntry.EntryType.OUT and quantity == 0):
                raise BusinessError(
                    "Dòng phiếu xuất/kiểm kê không hợp lệ",
                    errors={"items": ["Phiếu xuất cần số lượng dương; tồn kiểm kê có thể bằng 0"]},
                )
            normalized.append(
                {
                    "variant": variants[str(item["variant_id"])],
